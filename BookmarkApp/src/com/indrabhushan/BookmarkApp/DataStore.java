...
public class DataStore {
	...
	private static void loadWebLinks(Statement stmt) throws SQLException {
		...
		while (rs.next()) {
			...
			/*
			 * int kid_friendly_status = rs.getInt("kid+friendly_status"); KidFriendlyStatus
			 * kidFriendlyStatus = KidFriendlyStatus.values()[kid_friendly_status];
			 */
	...
	private static void loadUsers(Statement stmt) throws SQLException {
		...
		while (rs.next()) {
			...
			User user = UserManager.getInstance().createUser(id, email, password, first_name, last_name, gender,
					userType);
			users.add(user);
	...
	private static void loadBooks(Statement stmt) throws SQLException {
		...
		while (rs.next()) {
			 ...
			 * System.out.println("createdDate: " + createdDate); Timestamp timeStamp =
			 * rs.getTimestamp(8); System.out.println("timeStamp: " + timeStamp);
